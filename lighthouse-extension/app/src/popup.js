 ...
 */
function onGenerateReportButtonClick(background, selectedTags) {
  showRunningSubpage();
 ...
 * for the aggregation tags.
 * @param {!Window} background Reference to the extension's background page.
 * @param {!Object<boolean>} selectedTags
 */
function generateOptionsList(background, selectedTags) {
  const frag = document.createDocumentFragment();

  background.getDefaultAggregationTags().forEach(defaultTag => {
...
function initPopup() {
  getBackgroundPage.then(background => {
    ...
    background.loadSettings().then(settings => {
      generateOptionsList(background, settings.selectedTags);

      document.querySelector('.setting-disable-extensions').checked = settings.disableExtensions;

      const generateReportButton = document.getElementById('generate-report');
      generateReportButton.addEventListener('click', () => {
        onGenerateReportButtonClick(background, settings.selectedTags);
      });
    });
    ...
    okButton.addEventListener('click', () => {
      ...
      const selectedTags = [...optionsEl.querySelectorAll('input')].map(elem => {
        return {
          id: elem.name,
          value: elem.checked
        };
      ...
      background.saveSettings({selectedTags, disableExtensions});
